/**
 * Shared shape definitions for token-binding extraction. Every scanner that
 * lands a "this CSS property is bound to this token" fact has to file it
 * under the same property key or cross-section comparisons fail silently:
 *
 *   - the addon's PostCSS scanner of consumer `.css` files (Node, build-time)
 *   - the addon's ts-morph scanner of consumer `.tsx` files (Node, build-time)
 *   - the addon's DOM scanner of the rendered story (browser, runtime)
 *   - `tailwind.ts` in this package (utility class → token)
 *
 * Anything that varies by scanner — which shorthand expands to which
 * longhands, which composite values to pull a token out of, which key to
 * file the result under — lives here as the single source of truth.
 *
 * Previously this file lived in `storybook-sync-addon/src/binding-shape.ts`.
 * It moved here in core v0.0.2 because the Tailwind mapper needs the same
 * tables and duplicating them was the exact bug class the file exists to
 * prevent. The addon re-exports these names from its own
 * `src/binding-shape.ts` so existing import paths keep resolving.
 *
 * Pure data + pure functions; no runtime dependencies (see package README).
 */
