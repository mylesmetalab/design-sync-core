/**
 * Colour-space conversion and parsing, shared by every consumer.
 *
 * `getComputedStyle()` hands back whatever colour space the author wrote when a
 * value can't be represented in legacy `rgb()`. Modern shadcn / Tailwind v4
 * themes ship `oklch()` by default, so a *correct* themed colour reads as a
 * mismatch against any producer that emits `rgb()` — which is what Figma's REST
 * API does. Everything here exists to fold those into one comparable form.
 *
 * ### Conversion chain
 * OKLCh → OKLab (polar → rectangular) → LMS' → LMS (cube) → linear sRGB
 * (matrix) → gamma-encoded sRGB (sRGB transfer function) → 0-255, clamped.
 *
 * The OKLab ↔ linear-sRGB matrices are verbatim from Björn Ottosson's reference
 * implementation (https://bottosson.github.io/posts/oklab/), which is what CSS
 * Color 4 normatively describes.
 *
 * Out-of-gamut inputs (common — OKLCh addresses colours sRGB cannot) clamp per
 * channel rather than throwing. That is lossy: two different out-of-gamut
 * colours can fold to the same sRGB value.
 *
 * ## Why this is here rather than in each consumer
 *
 * It was forked between `storybook-design-sync` and `storybook-design-inspector`
 * for months. Each file carried a note saying to lift it into core "if a third
 * consumer appears" — a threshold that never arrived, so two copies drifted
 * unwatched while the condition for fixing it stayed unmet.
 *
 * They had **not** drifted, and that was established before merging rather than
 * assumed: a differential test compared both implementations across 22 parse
 * inputs and a 2,376-point OKLCh grid (L×C×H) and found exact agreement. So this
 * module is behaviour-preserving for both consumers by measurement, not by
 * inspection.
 *
 * What stays in the consumers is what differs: each one's **canonical form**.
 * The addon folds to `rgb(R,G,B)` for drift comparison; the inspector folds to
 * hex for display. That is a product decision, not colour maths.
 */
...
/**
 * True when `value` looks like one of the modern colour notations this module
 * knows how to convert. Cheap prefix test for callers that gate on shape.
 */
export function isModernColorNotation(value: string): boolean {
  return /^\s*(oklch|oklab|color)\s*\(/i.test(value);
}
